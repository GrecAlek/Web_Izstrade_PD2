...
<form method="post" action="{{ $album->exists ? '/albums/patch/' . $album->id : '/albums/put' }}" enctype="multipart/form-data">
 ...
 id="album-rapper"
 name="rapper_id"
 class="form-select @error('rapper_id') is-invalid @enderror"
 >
 <option value="">Norādiet Reperi!</option>
 @foreach($rappers as $rapper)
 <option
 value="{{ $rapper->id }}"
 @if ($rapper->id == old('rapper_id', $album->rapper->id ?? false)) selected @endif
 >{{ $rapper->name }}</option>
 @endforeach
 </select>
 @error('rapper_id')
 <p class="invalid-feedback">{{ $errors->first('rapper_id') }}</p>
 @enderror
 </div>
 <div class="mb-3">
